...
const TABLE_COURSES = process.env.NEXT_PUBLIC_AIRTABLE_COURSES_TABLE;

export async function GET() {
    try {
        // Fetch records from Airtable with the correct field names
        const records = await base(TABLE_COURSES).select({
            fields: [
                "Course Title", "Description", "Level", "Duration", "Number of Students", "Rating (1-5)", "Course Image", "External Link"
            ]
        }).all()
