...
export async function POST(req) {
    try {
        const { heading, image, location, date, time, honors, description, url } = await req.json()

        // Check for missing required fields
        if (!heading || !image || !location || !date || !time || !honors || !description || !url) {
        ...
        const newEvent = await base(TABLE_EVENTS).create({
            ...
            Honors: honors, // Note: API uses "Honors" with capital H
        ...
        return new Response(
            JSON.stringify({
                ...
                honors: newEvent.fields["Honors"], // Note: API uses "Honors" with capital H
...
export async function PUT(req) {
    try {
        const { id, heading, image, location, date, time, honors, description, url } = await req.json()
        ...
        return new Response(
            JSON.stringify({
                ...
                honors: updatedEvent[0].fields["Honors"], // Note: API uses "Honors" with capital H
